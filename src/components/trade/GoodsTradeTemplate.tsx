...
import {
  getCategoryListApi,
  getTradeListApi,
} from '../../core/redux/post/tradeSlice';
...
const GoodsTradeTemplate = (): ReactElement => {
  ...
  const [bestTradeList, setBestTradeList] = useState<any>([]);
  ...
  useEffect(() => {
    ...
    // 카테고리 목록 데이터 페칭
    appDispatch(getCategoryListApi(initCategoryList));
  }, []);

  const getBestTradeList = async () => {
    const resultList = (await tradeAPI.getBestTradeList()).data.data;
    setBestTradeList(resultList);
  };
